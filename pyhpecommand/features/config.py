...
class Config(object):
    ...
    """
    def __init__(self, device, filename, server_ip):
        self.device = device
...
#        time.sleep(.5)
    ...
    def build(self, stage=False):
        '''
        """Stage or copies a configfile to flash and renames it to startup.cfg.
        ...
        '''
        responses = []
        # Copy the startup cfg to saftey_file.cfg
        responses.append(self.device.save('safety_file.cfg'))
        # Build a list of commands
        cmds = []
        cmds.append('quit')
        cmds.append('tftp {0} get {1}'.format(self.server_ip,self.basename))
        cmds.append('copy my.cfg startup.cfg')
        cmds.append('startup saved-configuration startup.cfg main')
        #cmds.append('reboot')
        # raw string output taken directly from the switch showing the diff
        responses.append(self.device.cli_config(cmds))
        return responses
